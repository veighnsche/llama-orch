  ...
  <header class="nav" role="banner">
    <nav class="nav-inner" :aria-label="$t('a11y.navPrimary')">
      ...
      <ul class="links" role="menubar">
        <li role="none">
          <RouterLink role="menuitem" to="/public-tap">
            {{ $t('nav.publicTap') }}
          </RouterLink>
        </li>
        <li role="none">
          <RouterLink role="menuitem" to="/private-tap">
            {{ $t('nav.privateTap') }}
          </RouterLink>
        </li>
        <li role="none">
          <RouterLink role="menuitem" to="/pricing">
            {{ $t('nav.pricing') }}
          </RouterLink>
        </li>
        <li role="none">
          <RouterLink role="menuitem" to="/proof">
            {{ $t('nav.proof') }}
          </RouterLink>
        </li>
        <li role="none">
          <RouterLink role="menuitem" to="/faqs">
            {{ $t('nav.faqs') }}
          </RouterLink>
        </li>
        <li role="none">
          <RouterLink role="menuitem" to="/about">
            {{ $t('nav.about') }}
          </RouterLink>
        </li>
        <li role="none">
          <RouterLink role="menuitem" to="/contact">
            {{ $t('nav.contact') }}
          </RouterLink>
        </li>
      </ul>
      ...
      <div v-if="open" class="drawer" @click.self="open = false">
        <div class="drawer-panel">
          <ul class="drawer-links">
            <li>
              <RouterLink to="/public-tap" @click="open = false">
                {{ $t('nav.publicTap') }}
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/private-tap" @click="open = false">
                {{ $t('nav.privateTap') }}
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/pricing" @click="open = false">
                {{ $t('nav.pricing') }}
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/proof" @click="open = false">
                {{ $t('nav.proof') }}
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/faqs" @click="open = false">
                {{ $t('nav.faqs') }}
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/about" @click="open = false">
                {{ $t('nav.about') }}
              </RouterLink>
            </li>
            <li>
              <RouterLink to="/contact" @click="open = false">
                {{ $t('nav.contact') }}
              </RouterLink>
            </li>
          </ul>
          <div class="drawer-ops">
            <LanguageSwitcher />
            <Button as="router-link" to="/service-menu" variant="primary" @click="open = false">
              {{ $t('nav.serviceMenu', 'Service menu') }}
            </Button>
          </div>
        </div>
...
.brand {
  ...
  align-items: center;
  gap: 0.55rem;
  padding: 0 0.5rem; /* horizontal padding = better hitbox */
...
.brand-mark {
  ...
  flex: 0 0 auto;
  translate: 0 0.5px; /* tiny baseline nudge for optical alignment */
}
...
.drawer-links :global(.router-link-active) {
  background: var(--surface);
}
.drawer-ops {
  margin-top: 0.5rem;
...
@media (max-width: 919.98px) {
  ...
  .right :deep(.ui-btn) {
    display: none;
  } /* keep topbar tight on small screens; CTA moves into drawer */
}
