...
**Policy location**: `bin/queen-rbee/src/config.rs` (centralized)
...
- Extract policies (hardcoded → `queen-rbee/src/config.rs`)
- Enforce style (rustfmt + clippy, zero warnings)

**Folder Structure** (3-click rule):
```
✅ Domain-driven: queen-rbee/src/queue/, queen-rbee/src/scheduling/
...
**Policy System** (`bin/queen-rbee/src/config.rs`):
```rust
// Queue policies
...
- Centralize policies in queen-rbee (YAML, Rhai, env vars)
