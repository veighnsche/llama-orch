...
pub async fn then_output_masks_x_api_key(world: &mut BddWorld) {
...
pub async fn then_output_not_contains_raw_token(world: &mut BddWorld) {
...
pub async fn given_auth_token_unset(_world: &mut BddWorld) {
...
pub async fn when_apply_with_bearer(world: &mut BddWorld) {
...
pub async fn then_request_has_no_auth_header(world: &mut BddWorld) {
    let headers = world.last_headers.as_ref().expect("expected headers");
...
pub async fn when_get_client_twice(world: &mut BddWorld) {
...
pub async fn then_same_client(world: &mut BddWorld) {
...
pub async fn given_transient_upstream_503_succeeds(_world: &mut BddWorld) {
    // Setup would configure a stub server; placeholder for now.
}

#[when(regex = r"^I invoke with_retries around an idempotent request$")]
pub async fn when_invoke_with_retries(world: &mut BddWorld) {
...
pub async fn given_upstream_400(_world: &mut BddWorld) {}

#[then(regex = r"^no retry occurs$")]
pub async fn then_no_retry_occurs(world: &mut BddWorld) {
    let mut policy = RetryPolicy::default();
    policy.seed = Some(11);
...
pub async fn given_body_stream_started_token_metrics_end(world: &mut BddWorld) {
...
pub async fn then_ordering_preserved_and_token_indices_increasing(world: &mut BddWorld) {
...
pub async fn when_inspect_client_defaults(world: &mut BddWorld) {
...
pub async fn then_timeouts_approximate_defaults(world: &mut BddWorld) {
...
pub async fn then_tls_verification_on_by_default(_world: &mut BddWorld) {
    let cfg = default_config();
...
pub async fn then_h2_keepalive_enabled_when_alpn(_world: &mut BddWorld) {
