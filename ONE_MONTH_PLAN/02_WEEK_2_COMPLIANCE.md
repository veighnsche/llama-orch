...
- ✅ **Already integrated** in orchestratord (Week 1)
...
cat /var/log/llorch/audit/orchestratord/*.audit
```
...
✅ Hash chain integrity
✅ Flush modes (Immediate, Batched, Hybrid)
✅ Integrated in orchestratord (Week 1)
...
```rust
// In orchestratord/src/middleware.rs
use axum::{
    extract::Request,
    middleware::Next,
    response::Response,
...
**Task 3: Update llorch-candled registration (1 hour)**
```rust
...
# Spawn EU worker
LLORCH_REGION=EU llorch pool worker spawn metal --model tinyllama --host mac.home.arpa

# Spawn US worker (for testing)
LLORCH_REGION=US llorch pool worker spawn metal --model tinyllama --host workstation.home.arpa
...
llorch-candled --worker-id worker-1 --model model.gguf
...
// TODO: Wire up SSE streaming from orchestratord
// For now, mock data
...
**Task 3: Add CORS to orchestratord (30 min)**
```rust
// In orchestratord/src/main.rs
...
# Serve static files from orchestratord
# Add static file serving to orchestratord
```
