...
<!-- TEAM-FE-006: Implemented FeaturesSection component -->
<script setup lang="ts">
import { ref, computed } from 'vue'
import { Code, Cpu, Gauge, Zap } from 'lucide-vue-next'
import Tabs from '~/stories/atoms/Tabs/Tabs.vue'
import TabsList from '~/stories/atoms/Tabs/TabsList.vue'
import TabsTrigger from '~/stories/atoms/Tabs/TabsTrigger.vue'
import TabsContent from '~/stories/atoms/Tabs/TabsContent.vue'
  ...
  <section class="py-24 bg-secondary">
    <div class="container mx-auto px-4">
      ...
      <div class="max-w-5xl mx-auto">
        <Tabs v-model="activeTab" default-value="api" class="w-full">
          ...
          <TabsContent value="api" class="mt-8">
            <div class="bg-card border border-border rounded-lg p-8 space-y-6">
              <div>
                <h3 class="text-2xl font-bold text-card-foreground mb-3">OpenAI-Compatible API</h3>
              ...
              <div class="bg-card border border-border rounded-lg p-6 font-mono text-sm">
                ...
                <div class="text-green-600 mt-2">export OPENAI_API_KEY=sk-...</div>
                <div class="text-muted-foreground mt-4"># After: Using rbee (same code!)</div>
                <div class="text-green-600 mt-2">export OPENAI_API_BASE=http://localhost:8080/v1</div>
              </div>

              <div class="bg-green-100 border border-green-200 rounded-lg p-4">
                <p class="text-green-900 font-medium">✓ No code changes. Just point to localhost.</p>
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="gpu" class="mt-8">
            <div class="bg-card border border-border rounded-lg p-8 space-y-6">
              <div>
                <h3 class="text-2xl font-bold text-card-foreground mb-3">Multi-GPU Orchestration</h3>
                <p class="text-muted-foreground leading-relaxed">
          ...
          <TabsContent value="scheduler" class="mt-8">
            <div class="bg-card border border-border rounded-lg p-8 space-y-6">
              <div>
                <h3 class="text-2xl font-bold text-card-foreground mb-3">Programmable Rhai Scheduler</h3>
                <p class="text-muted-foreground leading-relaxed">
                  Write custom routing logic in Rhai. Route large models to multi-GPU setups, image generation to CUDA, everything else to cheapest.
                </p>
              </div>

              <div class="bg-card border border-border rounded-lg p-6 font-mono text-sm">
                <div class="text-muted-foreground">// Custom routing logic</div>
                <div class="text-primary mt-2">if <span class="text-foreground">task.model.contains("70b") {</span></div>
                <div class="text-foreground pl-4">route_to(<span class="text-green-600">"multi-gpu-cluster"</span>)</div>
                <div class="text-foreground">}</div>
                <div class="text-primary mt-2">else if <span class="text-foreground">task.type == "image" {</span></div>
                <div class="text-foreground pl-4">route_to(<span class="text-green-600">"cuda-only"</span>)</div>
                <div class="text-foreground">}</div>
                <div class="text-primary mt-2">else <span class="text-foreground">{</span></div>
                <div class="text-foreground pl-4">route_to(<span class="text-green-600">"cheapest"</span>)</div>
                <div class="text-foreground">}</div>
              </div>

              <div class="bg-accent border border-border rounded-lg p-4">
                <p class="text-accent-foreground font-medium">✓ Optimize for cost, latency, or compliance—your rules.</p>
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="sse" class="mt-8">
            <div class="bg-card border border-border rounded-lg p-8 space-y-6">
              <div>
                <h3 class="text-2xl font-bold text-card-foreground mb-3">Task-Based API with SSE</h3>
                <p class="text-muted-foreground leading-relaxed">
                  Real-time progress updates. See model loading, token generation, and cost tracking as it happens.
                </p>
              </div>

              <div class="bg-card border border-border rounded-lg p-6 font-mono text-sm space-y-2">
