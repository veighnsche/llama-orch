...
export function PlaybookItem({ icon, color, title, checkCount, severityDots, description, checks, footer }: PlaybookItemProps) {
  return (
...
export function PlaybookHeader({ title, description, filterCategories, onExpandAll, onCollapseAll }: PlaybookHeaderProps) {
  return (
    <div className="flex items-center justify-between gap-3 px-5 py-4 border-b border-border">
      ...
      <div className="flex items-center gap-2">
        {filterCategories.map((category) => (
          <button key={category} className="px-2.5 py-1 rounded-md text-xs bg-muted hover:bg-muted/70 transition-colors">
            {category}
          </button>
        ))}
        <button onClick={onExpandAll} className="ml-2 px-2.5 py-1 rounded-md text-xs border border-border hover:bg-muted transition-colors">
          Expand all
        </button>
        <button onClick={onCollapseAll} className="px-2.5 py-1 rounded-md text-xs border border-border hover:bg-muted transition-colors">
          Collapse all
        </button>
      </div>
    </div>
