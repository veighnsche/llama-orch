...
import { AlertTriangle, Network, Server, Activity } from "lucide-react";
...
export const Default: Story = {
  args: {
    ...
    description:
      "Detects timeouts, auth failures, HTTP errors; retries with exponential backoff.",
    checks: [
      {
...
export const ResourceErrors: Story = {
  args: {
    ...
    description:
      "Fail fast on RAM/VRAM limits with safe aborts and pre-download disk checks.",
    checks: [
      {
        severity: "destructive",
...
export const ModelBackend: Story = {
  args: {
    ...
    severityDots: ["destructive", "primary", "primary", "chart-2"],
    description:
      "Validate model presence, credentials, and ready inference endpoints.",
    checks: [
      {
        severity: "destructive",
...
export const ProcessLifecycle: Story = {
  args: {
    ...
    description:
      "Watch workers from startup to shutdown with safe termination.",
    checks: [
      {
        severity: "destructive",
...
export const FullPlaybook: Story = {
  render: () => {
    ...
    useEffect(() => {
      const allOpen = Object.values(detailsRefs.current).every(
        (ref) => ref?.open
      );
    ...
    const visibleCategories =
      selectedCategories.length === 0
        ...
        : categories.filter((cat) =>
            selectedCategories.includes(cat.title.split(" ")[0])
          );
        ...
        <div className="rounded-2xl border border-border bg-card overflow-hidden">
          ...
          {visibleCategories.map((category) => (
            <PlaybookItem
              key={category.id}
              {...category}
              onToggle={(isOpen) => handleToggle(category.id, isOpen)}
            />
          ))}
        </div>
      </div>
