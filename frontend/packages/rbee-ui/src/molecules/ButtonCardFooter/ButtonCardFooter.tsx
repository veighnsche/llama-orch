...
const buttonCardFooterVariants = cva('sticky bottom-0 z-10 flex flex-col gap-3', {
  variants: {
    variant: {
      default: '',
      elevated: 'shadow-lg',
...
const ButtonCardFooter = React.forwardRef<HTMLDivElement, ButtonCardFooterProps>(
  (
    {
      className,
      variant,
    ...
    return (
      ...
      >
        ...
        <a href={href} className="w-full">
          <Button className={buttonVariants({ color: buttonColor })} aria-describedby={ariaDescribedBy}>
            {buttonText}
            {showArrow && <ArrowRight className="ml-2 h-4 w-4" aria-hidden="true" />}
          </Button>
        </a>
      </div>
    )
