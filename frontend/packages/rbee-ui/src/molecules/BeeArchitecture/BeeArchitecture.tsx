...
export type BeeTopology =
  ...
  | {
      mode: "multi-host";
      hosts: Array<{ hostLabel: string; workers: WorkerNode[] }>;
    };
...
export function BeeArchitecture({ topology, className }: BeeArchitectureProps) {
  ...
  const getFigcaption = () => {
    if (topology.mode === "single-pc") {
      ...
      return `Topology diagram: queen-rbee orchestrates ${hostCount} PC${
        hostCount > 1 ? "s" : ""
      }, each with its own rbee-hive managing local devices.`;
    }
  };
  ...
  return (
    <figure
      className={cn(
        "mx-auto mt-16 max-w-3xl rounded-xl border border-border bg-card p-8",
        className
      )}
    >
      <figcaption className="sr-only">{getFigcaption()}</figcaption>
      <h3 className="mb-6 text-center text-xl font-semibold text-card-foreground">
