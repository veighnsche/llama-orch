...
const bulletContainerVariants = cva('h-6 w-6 flex items-center justify-center flex-shrink-0 mt-0.5', {
  variants: {
    color: {
      primary: '',
      'chart-1': '',
...
export interface BulletListItemProps extends VariantProps<typeof bulletListItemVariants> {
  /** Item title */
  title: string
...
export function BulletListItem({
  ...
  const renderBullet = () => {
    const effectiveVariant =
      (variant === 'check' || variant === 'arrow') && !showPlate ? (`${variant}-plain` as const) : variant
    ...
    switch (variant) {
      ...
      case 'arrow':
        return <div className={bulletContainerVariants({ color, variant: effectiveVariant })}>→</div>
    }
  }
