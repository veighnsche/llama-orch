...
export function FooterCTA({ message, ctas, className }: FooterCTAProps) {
  ...
  return (
    <div className={cn('mt-6 flex flex-col sm:flex-row gap-3 justify-center items-center', className)}>
      {message && <p className="text-sm text-muted-foreground text-center sm:text-left font-sans">{message}</p>}
      {ctas && ctas.length > 0 && (
