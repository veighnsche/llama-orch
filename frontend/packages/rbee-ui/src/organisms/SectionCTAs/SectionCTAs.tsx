 ...
 * />
 */
export function SectionCTAs({ label, primary, secondary, caption, className }: SectionCTAsProps) {
  ...
  return (
    <div className={cn('text-center', className)}>
      {label && <p className="mb-4 text-sm font-medium text-muted-foreground">{label}</p>}
      ...
      {caption && (
        <p className={cn('text-sm text-muted-foreground font-sans text-center', (primary || secondary) && 'mt-4')}>
          {caption}
        </p>
      )}
    </div>
