          ...
          <div className="grid gap-8 lg:grid-cols-[320px_minmax(0,1fr)]">
            ...
            <div className="lg:sticky lg:top-24 self-start space-y-6">
              ...
              <TabsList aria-label={title}>
                {items.map((item) => (
                  <TabsTrigger key={item.id} value={item.id} className="flex-col lg:flex-row items-start lg:items-center">
                    <span className="flex items-center gap-3 w-full">
                      <span className="size-4 text-muted-foreground group-data-[state=active]:text-primary" aria-hidden="true">
                        {item.icon}
                      </span>
                    ...
                    {item.example && (
                      <>
                        {(item.example.kind === 'terminal' || item.example.kind === 'code') && (
                          <ConsoleOutput
                            showChrome
                            title={item.example.title || (item.example.kind === 'terminal' ? 'terminal' : item.example.language || 'code')}
                            background="dark"
                            copyable
