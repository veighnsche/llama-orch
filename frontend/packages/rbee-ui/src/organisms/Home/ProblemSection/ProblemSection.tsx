...
function ProblemCard({ icon, title, body, tag, tone = 'destructive', delay }: ProblemItem & { delay?: string }) {
	...
	return (
		...
		>
			...
			{tag && (
				<span
					className={cn('mt-3 inline-flex rounded-full px-2.5 py-1 text-xs tabular-nums', styles.tagBg, styles.tagText)}
				>
					{tag}
				</span>
			)}
		...
		>
			<div className="mx-auto max-w-7xl">
				...
				<div className={cn('grid gap-6 sm:gap-7 md:grid-cols-3', gridClassName)}>
					{items.map((item, idx) => (
						<ProblemCard key={item.title} {...item} delay={['delay-75', 'delay-150', 'delay-200'][idx]} />
					))}
				</div>
