...
import { SectionContainer, StatsGrid, TestimonialCard, type StatItem } from '@rbee/ui/molecules'
...
export function TestimonialsSection({ title, subtitle, testimonials, stats, id, className }: TestimonialsSectionProps) {
	...
	return (
		<SectionContainer
			...
			maxWidth="7xl"
			paddingY="2xl"
			className={cn('border-b border-border motion-safe:animate-in motion-safe:fade-in-50 motion-safe:duration-400', className)}
			headingId={id}
		>
			...
			{/* Stats row */}
			{stats && stats.length > 0 && <StatsGrid stats={stats} variant="cards" columns={4} className="mx-auto mt-12 max-w-4xl" />}
		</SectionContainer>
	)
}
