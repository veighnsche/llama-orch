	...
	const renderBadge = (included: boolean | string, plan: string) => {
		if (typeof included === 'string') {
			return (
				<Badge
					variant="outline"
					className="rounded-full px-2.5 py-1 text-xs border bg-secondary text-secondary-foreground"
				>
					{plan}: {included}
				</Badge>
			)
						...
						<table className="w-full">
							...
							<thead>
								<tr className="border-b border-border">
									<th
										scope="col"
										className="text-left px-4 py-3 font-semibold text-foreground sticky left-0 z-10 bg-card"
									>
										Feature
									</th>
