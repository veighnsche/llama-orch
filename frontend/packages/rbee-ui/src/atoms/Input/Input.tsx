...
function Input({ className, type, ...props }: React.ComponentProps<'input'>) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        'file:text-foreground placeholder:text-slate-400 dark:placeholder:text-[#8b9bb0] selection:bg-primary selection:text-primary-foreground bg-white dark:bg-[color:var(--background)] border-input h-9 w-full min-w-0 rounded-md border px-3 py-1 text-base transition-[color,box-shadow,border-color] file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:cursor-not-allowed md:text-sm',
