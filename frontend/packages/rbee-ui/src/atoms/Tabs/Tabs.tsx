...
function TabsTrigger({ className, ...props }: React.ComponentProps<typeof TabsPrimitive.Trigger>) {
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        'group inline-flex lg:flex w-max lg:w-full items-start justify-start gap-2 rounded-md border border-transparent px-3 py-3 text-left text-sm font-medium snap-start text-foreground dark:text-muted-foreground transition-all hover:bg-muted/60 hover:-translate-y-0.5 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:border-border data-[state=active]:shadow-sm data-[state=active]:text-foreground [&_svg]:pointer-events-none [&_svg]:shrink-0',
        focusRing,
        className,
      )}
      {...props}
