...
const Card = React.forwardRef<HTMLDivElement, CardProps>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-slot="card"
      className={cn('bg-card text-card-foreground flex flex-col rounded-xl border border-border [box-shadow:var(--shadow-sm)]', className)}
      {...props}
    />
  )
})
...
const CardTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => {
    return <h3 ref={ref} data-slot="card-title" className={cn('leading-tight font-semibold', className)} {...props} />
  },
)
