 ...
 * ```
 */
export function CodeSnippet({ children, variant = 'inline', className }: CodeSnippetProps) {
  if (variant === 'inline') {
    return (
      <code className={cn('rounded-md bg-muted px-1.5 py-0.5 text-sm text-foreground font-mono', className)}>
        {children}
      </code>
    )
  }
  ...
  return (
    <div className={cn('overflow-x-auto rounded-lg border border-border bg-muted p-3', className)}>
