...
pub async fn submit_and_stream_job(
    ...
    let submit_client = reqwest::Client::builder().timeout(Duration::from_secs(10)).build()?;

    // Submit job to queen
    let res =
        submit_client.post(format!("{}/v1/jobs", queen_url)).json(&job_payload).send().await?;
    ...
    let stream_result = TimeoutEnforcer::new(Duration::from_secs(30))
        ...
        .enforce(async move {
            ...
            let sse_client = reqwest::Client::builder().timeout(Duration::from_secs(10)).build()?;
                ...
                for line in text.lines() {
                    ...
                    if let Some(data) = line.strip_prefix("data: ") {
                        ...
                        if data.contains("failed:")
                            || data.contains("Job") && data.contains("failed")
                        {
                            job_failed = true;
                        }
