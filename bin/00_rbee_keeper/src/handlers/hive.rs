...
pub async fn handle_hive(action: HiveAction, queen_url: &str) -> Result<()> {
    match action {
        ...
        HiveAction::Start { port } => {
            ...
            let binary = daemon_lifecycle::DaemonManager::find_binary("rbee-hive")?;
            let base_url = format!("http://localhost:{}", port);
            let args = vec![
                "--port".to_string(), port.to_string(),
                "--queen-url".to_string(), queen_url.to_string(),
                "--hive-id".to_string(), "localhost".to_string(),
            ];
            let config = daemon_lifecycle::HttpDaemonConfig::new("rbee-hive", binary, &base_url)
                .with_args(args);
        ...
        HiveAction::Stop { port } => {
            ...
            let config = HttpDaemonConfig::new(
                "rbee-hive",
                PathBuf::from("~/.local/bin/rbee-hive"),
                base_url,
            );
