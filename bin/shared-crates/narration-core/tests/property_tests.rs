...
use observability_narration_core::{
    generate_correlation_id, redact_secrets, sanitize_crlf, validate_correlation_id,
    RedactionPolicy,
};
...
fn property_api_keys_never_leak() {
    let test_cases =
        vec!["api_key=secret123", "api-key: secret456", "apikey=secret789", "API_KEY=SECRET"];
...
fn property_jwt_tokens_never_leak() {
    ...
    let test_cases = vec![jwt, &with_auth, &with_token];
...
fn property_redaction_performance() {
    ...
    let test_cases = vec![&typical_message, &with_bearer, &with_api_key];
...
fn property_zero_width_characters_removed() {
    ...
    for zw_char in zero_width_chars {
        ...
        assert_eq!(
            ...
            "Zero-width character not removed: U+{:04X}",
            zw_char as u32
        );
    }
}
