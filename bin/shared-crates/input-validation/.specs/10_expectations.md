...
- `bin/orchestratord` — Task requests (model_ref, task_id, prompts)
...
**Required by**: All services (orchestratord, pool-managerd, worker-orcd, vram-residency)
...
- **orchestratord**: Validate `task_id`, `session_id`, `correlation_id` (max 256 chars)
...
**Required by**: orchestratord, catalog-core, pool-managerd
...
**Use cases**:
- **orchestratord**: Validate `model_ref` in task creation (SECURITY_AUDIT #10)
...
**Required by**: worker-orcd (api crate), orchestratord
...
- **orchestratord**: Validate prompts in task creation
...
**Required by**: worker-orcd (api, vram-residency), orchestratord
...
- **orchestratord**: Validate `ctx` (context length), `deadline_ms`
...
13. ⬜ Integration with orchestratord
