...
pub fn validate_path(path: impl AsRef<Path>, allowed_root: &Path) -> Result<PathBuf> {
    ...
    let path_str = path
        .to_str()
        .ok_or_else(|| ValidationError::InvalidCharacters { found: "[non-UTF8]".to_string() })?;
    ...
    if !canonical_root.is_dir() {
        return Err(ValidationError::Io("allowed_root must be a directory".to_string()));
    }

    // Canonicalize path (resolve .. and symlinks)
    // This is where symlink resolution happens
    // IMPORTANT: The path must exist for canonicalization to work
    let canonical = path.canonicalize().map_err(|e| ValidationError::Io(e.to_string()))?;
    ...
    let _ = canonical.to_str().ok_or_else(|| ValidationError::InvalidCharacters {
