...
mod tests {
    ...
    fn test_log_injection_prevented() {
        ...
        assert_eq!(
            sanitize_string("text\x1b[31m[ERROR] Fake"),
            Err(ValidationError::AnsiEscape)
        );
    }
    ...
    fn test_null_byte_positions() {
        // Null byte at start
        assert_eq!(
            sanitize_string("\0text"),
            Err(ValidationError::NullByte)
        );
        
        // Null byte at end
        assert_eq!(
            sanitize_string("text\0"),
            Err(ValidationError::NullByte)
        );
        
        // Multiple null bytes
        assert_eq!(
            sanitize_string("text\0\0null"),
            Err(ValidationError::NullByte)
        );
    }
    ...
    fn test_more_ansi_escapes() {
        ...
        assert_eq!(
            sanitize_string("text\x1b[2J"),
            Err(ValidationError::AnsiEscape)
        );
        
        // ANSI at start
        assert_eq!(
            sanitize_string("\x1b[31mred text"),
            Err(ValidationError::AnsiEscape)
        );
        ...
        assert_eq!(
            sanitize_string("\x1b[31mred\x1b[0m"),
            Err(ValidationError::AnsiEscape)
        );
    }
    ...
    fn test_early_termination_order() {
        ...
        assert_eq!(
            sanitize_string("text\0\x1b[31m"),
            Err(ValidationError::NullByte)
        );
        
        // ANSI checked before control characters
        assert_eq!(
            sanitize_string("text\x1b[31m\x01"),
            Err(ValidationError::AnsiEscape)
        );
    ...
    fn test_robustness_all_control_characters() {
        ...
        for i in 0u8..=31u8 {
            ...
            assert!(
                result.is_err(),
                "Should reject control character 0x{:02X} ({})", i, c.escape_debug()
            );
        }
    }
    ...
    fn test_robustness_zero_width_characters_allowed() {
        ...
        assert_eq!(
            sanitize_string("text\u{200B}zwsp").unwrap(),
            "text\u{200B}zwsp"
        );
        
        // ZERO WIDTH NON-JOINER (U+200C) - allowed for complex scripts
        assert_eq!(
            sanitize_string("text\u{200C}zwnj").unwrap(),
            "text\u{200C}zwnj"
        );
        
        // ZERO WIDTH JOINER (U+200D) - allowed for complex scripts
        assert_eq!(
            sanitize_string("text\u{200D}zwj").unwrap(),
            "text\u{200D}zwj"
        );
    }
    ...
    fn test_robustness_mixed_whitespace() {
        ...
        // Multiple tabs
        assert_eq!(
            sanitize_string("text\t\t\ttabs").unwrap(),
            "text\t\t\ttabs"
        );
    ...
    fn test_robustness_unicode_text() {
        ...
        // Mixed scripts
        assert_eq!(
            sanitize_string("Hello مرحبا 你好 🌍").unwrap(),
            "Hello مرحبا 你好 🌍"
        );
    }
    ...
    fn test_robustness_structured_log_safety() {
        ...
        assert_eq!(
            sanitize_string("{\"key\": \"value\"}").unwrap(),
            "{\"key\": \"value\"}"
        );
    ...
    fn test_robustness_validation_order() {
        ...
        assert_eq!(
            sanitize_string("text\0\x1b\x01"),
            Err(ValidationError::NullByte)
        );
        
        // ANSI checked before control chars
        assert_eq!(
            sanitize_string("text\x1b\x01"),
            Err(ValidationError::AnsiEscape)
        );
    ...
    fn test_robustness_real_world_log_messages() {
        ...
        for log in valid_logs {
            assert!(
                sanitize_string(log).is_ok(),
                "Should accept valid log: {}", log
            );
        }
    }
