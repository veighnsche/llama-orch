...
```
Client → orchestratord (50ms) → pool-managerd (30ms) → worker-orcd (20ms) → inference (4800ms)
Total: 4900ms
...
**Use cases**:
- orchestratord: Before enqueuing job
...
|-------|-----------------|
| **orchestratord** | Parse client deadlines, enforce before enqueue, forward to pool-managerd |
...
- orchestratord forwards: `X-Deadline: 2025-10-02T20:59:12Z` (same)
...
### Dear orchestratord, pool-managerd, worker-orcd,
...
- ⬜ **Integration with orchestratord**: Parse client deadlines, enforce before enqueue
