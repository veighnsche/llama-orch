...
use crate::{Result, SecretError, SecretKey};
use hkdf::Hkdf;
use sha2::Sha256;
...
pub fn derive_key_from_token(token: &str, domain: &[u8]) -> Result<SecretKey> {
    ...
    if domain.is_empty() {
        return Err(SecretError::KeyDerivation("domain separation string required".to_string()));
    }

    // HKDF-SHA256 with domain separation
    let hkdf = Hkdf::<Sha256>::new(None, token.as_bytes());
    let mut key = [0u8; 32];
    hkdf.expand(domain, &mut key).map_err(|e| SecretError::KeyDerivation(e.to_string()))?;
