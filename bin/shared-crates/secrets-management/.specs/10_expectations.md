...
**Current Implementation** (`bin/queen-rbee/src/app/auth_min.rs:41`):
```rust
...
**Also Used In**:
- `bin/queen-rbee/src/api/nodes.rs:26` — Duplicate auth logic
- `bin/queen-rbee/src/clients/pool_manager.rs:49` — Outbound client auth

### 2.2 Expected API (EXP-ORCH-2002)

**What queen-rbee expects from secrets-management**:
...
**Security Issue**: Same as queen-rbee (environment variable exposure)

### 3.2 Expected API (EXP-POOL-3002)

**What pool-managerd expects from secrets-management**:

**EXP-POOL-3002-R1**: Identical API to queen-rbee (consistency)
...
| Feature | queen-rbee | pool-managerd | vram-residency | worker-orcd | Priority |
|---------|---------------|---------------|----------------|-------------|----------|
...
**Phase 1 (M0)**: Core functionality for queen-rbee/pool-managerd
...
**End-to-End Tests**:
- ✅ queen-rbee authenticates pool-managerd requests
- ✅ pool-managerd authenticates queen-rbee requests
...
- `bin/queen-rbee/src/app/auth_min.rs` — Current token loading
