...
use crate::loaders::file::{load_key_from_file, load_secret_from_file};
use crate::{Result, Secret, SecretError, SecretKey};
use std::path::PathBuf;

/// Validate systemd credential name
///
/// # Security
///
/// - Rejects empty names
...
/// # Errors
///
/// Returns `SecretError::PathValidationFailed` if validation fails.
...
pub fn load_from_systemd_credential(name: &str) -> Result<Secret> {
    ...
    let creds_dir = std::env::var("CREDENTIALS_DIRECTORY").map_err(|_| {
        SecretError::SystemdCredentialNotFound("CREDENTIALS_DIRECTORY not set".to_string())
    })?;
    ...
    if !creds_path.is_absolute() {
        return Err(SecretError::PathValidationFailed(format!(
            "CREDENTIALS_DIRECTORY must be absolute path, got: {}",
            creds_dir
        )));
    }
    ...
    if !path.exists() {
        return Err(SecretError::SystemdCredentialNotFound(format!(
