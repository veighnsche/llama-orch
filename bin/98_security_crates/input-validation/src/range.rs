...
mod tests {
    ...
    fn test_out_of_range_rejected() {
        // Below min
        assert!(matches!(validate_range(-1, 0, 4), Err(ValidationError::OutOfRange { .. })));

        // At max (exclusive)
        assert!(matches!(validate_range(4, 0, 4), Err(ValidationError::OutOfRange { .. })));

        // Above max
        assert!(matches!(validate_range(5, 0, 4), Err(ValidationError::OutOfRange { .. })));
    }
