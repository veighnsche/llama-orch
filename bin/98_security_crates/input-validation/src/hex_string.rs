...
pub fn validate_hex_string(s: &str, expected_len: usize) -> Result<()> {
    ...
    if s.len() != expected_len {
        return Err(ValidationError::WrongLength { actual: s.len(), expected: expected_len });
    }
...
mod tests {
    ...
    fn test_null_byte_rejected() {
        assert_eq!(validate_hex_string("abc\0def", 7), Err(ValidationError::NullByte));
    }
    ...
    fn test_null_byte_positions() {
        // Null byte at start
        assert_eq!(validate_hex_string("\0abcdef", 7), Err(ValidationError::NullByte));

        // Null byte at end
        assert_eq!(validate_hex_string("abcdef\0", 7), Err(ValidationError::NullByte));
    }
