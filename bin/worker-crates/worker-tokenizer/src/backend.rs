...
use crate::{hf_json::HfJsonTokenizer, BPEDecoder, BPEEncoder, TokenizerError};
use std::path::Path;
...
pub enum Tokenizer {
    /// GGUF BPE tokenizer
    GgufBpe { encoder: BPEEncoder, decoder: BPEDecoder },
...
impl Tokenizer {
    ...
    pub fn from_gguf<P: AsRef<Path>>(path: P) -> Result<Self, TokenizerError> {
        ...
        let bos_token_id = metadata.bos_token_id().map_err(|e| {
            TokenizerError::LoadFailed(format!("Failed to extract BOS token: {}", e))
        })?;

        let eos_token_id = metadata.eos_token_id().map_err(|e| {
            TokenizerError::LoadFailed(format!("Failed to extract EOS token: {}", e))
        })?;
    ...
    pub fn encode(&self, text: &str, add_special_tokens: bool) -> Result<Vec<u32>, TokenizerError> {
        match self {
            ...
            Tokenizer::HfJson(hf) => hf.encode(text, add_special_tokens),
        }
    }
    ...
    pub fn decode(
        ...
        match self {
            ...
            Tokenizer::HfJson(hf) => hf.decode(token_ids, skip_special_tokens),
        }
    }
...
mod tests {
    ...
    fn test_backend_detection() {
        assert_eq!(TokenizerBackend::from_path("tokenizer.json"), TokenizerBackend::HfJson);
        assert_eq!(TokenizerBackend::from_path("model.gguf"), TokenizerBackend::GgufBpe);
    }
