...
- `end`: `{"tokens_out": <int>, "decode_time_ms": <int>}`
...
- Budget updates:
  - On stream end: spend `tokens_out`, spend time `decode_time_ms`, and spend cost using placeholder cost model `$0.000002 * tokens_out`
- Eviction:
...
`orchestratord/Cargo.toml` defines feature flags (defaults enable `server` and `artifacts`):
