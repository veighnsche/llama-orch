...
use pool_managerd::lifecycle::supervision::{
    BackoffPolicy, CircuitBreaker, CircuitState, CrashEvent, CrashReason, RestartRateLimiter,
};
...
fn test_backoff_exponential_progression() {
    ...
    let delays: Vec<u64> = (0..5).map(|_| policy.next_delay().as_millis() as u64).collect();
