...
fn test_drain_then_reload_cycle() {
    ...
    let drain_outcome = drain::execute_drain(drain_req, &mut registry).expect("drain failed");
...
fn test_multiple_drain_cycles() {
    ...
    for i in 0..3 {
        ...
        let outcome = drain::execute_drain(drain_req, &mut registry).expect("drain failed");
