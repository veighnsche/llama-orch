...
impl Config {
    ...
    pub fn from_env() -> Result<Self> {
        ...
        let node_config = if cloud_profile {
            Some(NodeConfig::from_env()?)
        } else {
            None
        };
...
impl NodeConfig {
    ...
    fn from_env() -> Result<Self> {
        let node_id = std::env::var("POOL_MANAGERD_NODE_ID")
            .unwrap_or_else(|_| hostname::get()
                .ok()
                .and_then(|h| h.into_string().ok())
                .unwrap_or_else(|| "unknown-node".to_string()));

        let machine_id = std::env::var("POOL_MANAGERD_MACHINE_ID")
