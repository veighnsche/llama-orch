...
fn test_build_config_is_debug() {
    let config = BuildConfig { daemon_name: "test-daemon".to_string(), target: None, job_id: None };
...
fn test_binary_path_with_target() {
    ...
    assert_eq!(binary_path, PathBuf::from("target/x86_64-unknown-linux-gnu/release/test-daemon"));
}
...
async fn test_build_with_empty_daemon_name() {
    ...
    let config = BuildConfig { daemon_name: "".to_string(), target: None, job_id: None };
...
async fn test_build_with_special_characters_in_name() {
    ...
    let config =
        BuildConfig { daemon_name: "daemon-with-dashes".to_string(), target: None, job_id: None };
