...
pub fn validate_identifier(s: &str, max_len: usize) -> Result<()> {
    ...
    for c in s.chars() {
        ...
        if prev == '.' {
            return Err(ValidationError::InvalidCharacters { found: ".".to_string() });
        }
...
mod tests {
    ...
    fn test_null_byte_rejected() {
        assert_eq!(validate_identifier("shard\0null", 256), Err(ValidationError::NullByte));
    }
    ...
    fn test_null_byte_positions() {
        ...
        // Multiple null bytes
        assert_eq!(validate_identifier("shard\0\0null", 256), Err(ValidationError::NullByte));
    }
    ...
    fn test_robustness_null_byte_all_positions() {
        ...
        for i in 0..5 {
            ...
            assert_eq!(
                ...
                "Failed to detect null byte at position {}",
                i
            );
        }
    }
    ...
    fn test_robustness_validation_order() {
        ...
        assert_eq!(validate_identifier("\0../etc", 256), Err(ValidationError::NullByte));

        // Path traversal check happens before character validation
        assert_eq!(validate_identifier("../!@#", 256), Err(ValidationError::PathTraversal));
    }
}
