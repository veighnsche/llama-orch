...
pub fn validate_prompt(s: &str, max_len: usize) -> Result<()> {
    ...
    if char_count > max_len {
        return Err(ValidationError::TooLong { actual: char_count, max: max_len });
    }
...
mod tests {
    ...
    fn test_null_byte_rejected() {
        assert_eq!(validate_prompt("prompt\0null", 100_000), Err(ValidationError::NullByte));
    }
    ...
    fn test_null_byte_positions() {
        // Null byte at start
        assert_eq!(validate_prompt("\0prompt", 100_000), Err(ValidationError::NullByte));

        // Null byte at end
        assert_eq!(validate_prompt("prompt\0", 100_000), Err(ValidationError::NullByte));

        // Multiple null bytes
        assert_eq!(validate_prompt("prompt\0\0null", 100_000), Err(ValidationError::NullByte));
    }
    ...
    fn test_robustness_resource_exhaustion() {
        ...
        assert!(matches!(validate_prompt(&extreme, 100_000), Err(ValidationError::TooLong { .. })));
    }

    #[test]
    fn test_robustness_tokenizer_attacks() {
        // Null byte (could truncate in C tokenizers)
        assert_eq!(validate_prompt("prompt\0malicious", 100_000), Err(ValidationError::NullByte));
    ...
    fn test_robustness_validation_order() {
        ...
        let long = "a".repeat(100_001);
        assert!(matches!(validate_prompt(&long, 100_000), Err(ValidationError::TooLong { .. })));

        // Null byte check happens before control char check
        assert_eq!(validate_prompt("text\0\x07", 100_000), Err(ValidationError::NullByte));
    ...
    fn test_robustness_real_world_prompts() {
        ...
        for prompt in prompts {
            assert!(
                ...
                "Should accept real-world prompt: {}",
                prompt
            );
        }
    }
