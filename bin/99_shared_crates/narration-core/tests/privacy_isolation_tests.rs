...
async fn test_multi_tenant_isolation() {
    ...
    let mut rx_a = sse_sink::take_job_receiver(job_a)
        .expect("User A channel should exist");
    let mut rx_b = sse_sink::take_job_receiver(job_b)
        .expect("User B channel should exist");
    
    // User A's secret narration
    let ctx_a = context::NarrationContext::new()
        .with_job_id(job_a);
    ...
    let ctx_b = context::NarrationContext::new()
        .with_job_id(job_b);
...
async fn test_concurrent_jobs_isolation() {
    ...
    for job_id in &jobs {
        ...
        let rx = sse_sink::take_job_receiver(job_id)
    ...
    for (job_id, mut rx) in receivers {
        ...
        assert!(matches!(rx.try_recv(), Err(tokio::sync::mpsc::error::TryRecvError::Empty)),
            "Should have no more events for job {}", job_id);
    }
}
