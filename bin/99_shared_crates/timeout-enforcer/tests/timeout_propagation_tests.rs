...
async fn test_layered_timeouts() {
    ...
    let result = tokio::time::timeout(keeper_timeout, async {
...
async fn test_innermost_timeout_fires_first() {
    ...
    let result = tokio::time::timeout(outer_timeout, async {
        tokio::time::timeout(inner_timeout, async {
...
async fn test_multiple_sequential_timeouts() {
    ...
    let result1 = tokio::time::timeout(timeout, async {
        sleep(Duration::from_millis(100)).await;
        "completed-1"
    })
    .await;
    ...
    let result2 = tokio::time::timeout(timeout, async {
        sleep(Duration::from_millis(500)).await;
        "completed-2"
    })
    .await;
    ...
    let result3 = tokio::time::timeout(timeout, async {
        sleep(Duration::from_millis(100)).await;
        "completed-3"
    })
    .await;
