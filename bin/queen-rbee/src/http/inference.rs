...
use crate::http::types::{
    InferenceRequest, InferenceTaskRequest, ReadyResponse, WorkerSpawnResponse,
};
...
pub async fn handle_create_inference_task(
    ...
    let rbee_hive_url = if req.node == "localhost" {
        ...
        match ensure_local_rbee_hive_running().await {
            ...
            Err(e) => {
                ...
                return (
                    ...
                    format!("Failed to start rbee-hive: {}", e),
                )
                    .into_response();
            }
        }
        ...
        let node = match state.beehive_registry.get_node(&req.node).await {
            ...
            Ok(None) => {
                ...
                return (StatusCode::NOT_FOUND, format!("Node '{}' not registered", req.node))
                    .into_response();
            }
            Err(e) => {
                error!("Failed to query registry: {}", e);
                return (StatusCode::INTERNAL_SERVER_ERROR, format!("Registry error: {}", e))
                    .into_response();
            }
        };
        ...
        .await
    {
        ...
        Ok(resp) => {
            ...
            return (
                ...
                format!(
                    "Worker spawn failed: HTTP {} - {}. Check rbee-hive logs for details.",
                    status, body
                ),
            )
                .into_response();
        ...
        Err(e) => {
            ...
            return (
                StatusCode::SERVICE_UNAVAILABLE,
                format!(
                    "Worker spawn request failed: {}. Is rbee-hive running at {}?",
                    e, rbee_hive_url
                ),
            )
                .into_response();
        ...
        .await
    {
        ...
        Ok(resp) => {
            ...
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                format!("Inference failed: HTTP {}", status),
            )
                .into_response();
        }
        Err(e) => {
            error!("Failed to execute inference: {}", e);
            return (StatusCode::SERVICE_UNAVAILABLE, format!("Inference request failed: {}", e))
                .into_response();
        }
    };
...
async fn wait_for_rbee_hive_ready(url: &str) -> anyhow::Result<()> {
    ...
    for attempt in 0..max_retries {
            ...
            .await
        {
            ...
            Err(e) => {
                anyhow::bail!(
                    "Failed to connect to rbee-hive after {} attempts: {}",
                    max_retries,
                    e
                );
            }
            ...
            Ok(response) if response.status().is_success() => {
                match response.json::<ReadyResponse>().await {
                    Ok(ready) => {
                        if ready.ready {
                            info!(
                                "✅ Worker ready at {} (took {:.1}s, {} attempts)",
                                worker_url,
                                elapsed.as_secs_f32(),
                                attempt
                            );
                            ...
                            last_error =
                                Some(format!("Worker not ready yet (state: {})", ready.state));
                        }
                    }
                    Err(e) => {
...
/// Created by: TEAM-084
/// TEAM-114: Added deadline propagation
pub async fn handle_inference_request(
    State(state): State<AppState>,
    req: Request<Body>,
) -> impl IntoResponse {
    // TEAM-114: Extract deadline from request headers (or create default 60s deadline)
    let deadline = req
        .headers()
        .get("x-deadline")
        .and_then(|h| h.to_str().ok())
        .and_then(|s| Deadline::from_header(s).ok())
        .unwrap_or_else(|| Deadline::from_duration(std::time::Duration::from_secs(60)).unwrap());

    // TEAM-114: Check if deadline already expired
    if deadline.is_expired() {
        error!("Request deadline already expired");
    ...
    info!(
        "Received inference request: model={}",
        inference_req.model.as_deref().unwrap_or("default")
    );
    ...
    if let Some(worker) = idle_worker {
        ...
        // TEAM-114: Calculate timeout from remaining deadline
        ...
        match client
            .post(format!("{}/v1/inference", worker.url))
            .header("x-deadline", deadline.to_header_value()) // TEAM-114: Propagate deadline
            .json(&inference_req)
            .timeout(timeout) // TEAM-114: Use deadline-based timeout
            .send()
            .await
        {
            Ok(response) => {
            ...
            Err(e) => {
                ...
                (StatusCode::SERVICE_UNAVAILABLE, format!("Worker communication failed: {}", e))
                    .into_response()
            }
        }
    } else {
        error!("No idle workers available");
        (StatusCode::SERVICE_UNAVAILABLE, "No idle workers available").into_response()
    }
}
