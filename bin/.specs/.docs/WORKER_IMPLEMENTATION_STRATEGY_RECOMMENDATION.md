...
## Executive Summary
**Decision**: **Build custom worker-orcd with ModelAdapter pattern**
**Rationale**: Analysis confirms that building a custom worker is the only path that preserves the system's core value propositions. External engines would sacrifice 8-15 critical features including narration-core logging,  emission, VRAM-only enforcement, and API contract compliance.
...
- ** emission**: Standardized test artifacts (libs/ integration)
...
- ✅ ** support**: Direct integration with libs/
...
2. **❌  emission**: llama.cpp doesn't know about our  standard
   - Cannot emit to libs/ format
