...
fn bench_prefill(c: &mut Criterion) {
    ...
    for seq_len in [32, 128, 512, 1024] {
        ...
        group.bench_with_input(
            ...
            |b, (ids, cfg)| {
                b.iter(|| {
                    black_box(qwen_adapter.prefill(ids, cfg).unwrap())
                });
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, (ids, cfg)| {
                b.iter(|| {
                    black_box(phi3_adapter.prefill(ids, cfg).unwrap())
                });
            },
        );
    }
...
fn bench_decode(c: &mut Criterion) {
    ...
    group.bench_function("qwen_decode", |b| {
        b.iter(|| {
            black_box(qwen_adapter.decode(42, &config).unwrap())
        });
    });
    
    group.bench_function("phi3_decode", |b| {
        b.iter(|| {
            black_box(phi3_adapter.decode(42, &config).unwrap())
        });
    });
...
fn bench_generation_throughput(c: &mut Criterion) {
    ...
    for max_tokens in [10, 50, 100] {
        ...
        group.bench_with_input(
            ...
            |b, (ids, tokens, cfg)| {
                b.iter(|| {
                    black_box(qwen_adapter.generate(ids, *tokens, cfg).unwrap())
                });
            },
        );
    }
...
fn bench_vram_queries(c: &mut Criterion) {
    ...
    group.bench_function("vram_usage", |b| {
        b.iter(|| {
            black_box(qwen_adapter.vram_usage().unwrap())
        });
    });
    
    group.bench_function("vocab_size", |b| {
        b.iter(|| {
            black_box(qwen_adapter.vocab_size().unwrap())
        });
    });
    
    group.bench_function("hidden_dim", |b| {
        b.iter(|| {
            black_box(qwen_adapter.hidden_dim().unwrap())
        });
    });
