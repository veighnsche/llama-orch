...
async fn test_emoji_streaming() {
    ...
    let mut req = make_test_request("test-emoji", "List 5 emojis: 🎨", 50);
    req.temperature = 0.7;
    ...
    assert!(
        output.is_ascii()
            || output.chars().all(|c| c.is_alphanumeric() || c.is_whitespace() || !c.is_control())
    );
...
async fn test_multibyte_characters() {
    ...
    let mut req = make_test_request("test-multibyte", "Write in Japanese: こんにちは", 30);
    req.temperature = 0.7;
...
async fn test_mixed_scripts() {
    ...
    let mut req = make_test_request("test-mixed", "Mix English, 中文, and العربية", 40);
    req.temperature = 0.7;
...
fn test_utf8_validation() {
    ...
    let valid_sequences = vec!["Hello", "🎨", "こんにちは", "中文", "العربية", "Emoji: 🚀🎯✨"];
