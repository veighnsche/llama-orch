...
mod oom_recovery_tests {
    ...
    fn test_vram_oom_during_inference() {
        ...
        println!(
            "  Available after model: {:.2} GB",
            available_after_model / 1024.0 / 1024.0 / 1024.0
        );
        ...
        println!(
            "\n  Attempting KV cache allocation: {:.2} GB",
    ...
    fn test_oom_error_handling_cleanup() {
        ...
        for (name, size, should_succeed) in allocations {
            ...
            if should_succeed {
                ...
                assert!(
                    total_allocated.abs() < 1.0,
                    "Memory leak detected: {} bytes remaining",
                    total_allocated
                );
                break;
            }
    ...
    fn test_oom_recovery_metrics() {
        ...
        let avg_recovery = if oom_count > 0 { total_recovery_time / oom_count } else { 0 };
