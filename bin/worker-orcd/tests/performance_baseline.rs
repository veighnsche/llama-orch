...
async fn test_qwen_baseline_performance() {
    ...
    let mut req =
        make_test_request("perf-baseline-qwen", "Write a short story about a robot.", 100);
    req.temperature = 0.7;
    req.seed = Some(42);
...
async fn test_gpt_baseline_performance() {
    let harness = WorkerTestHarness::start(".test-models/gpt/gpt-oss-20b-mxfp4.gguf", 0)
        .await
        .expect("Failed to start worker");

    let mut req = make_test_request("perf-baseline-gpt", "Write a short story about a robot.", 100);
    req.temperature = 0.7;
    req.seed = Some(42);
...
async fn test_batch_performance() {
    ...
    for (i, prompt) in prompts.iter().enumerate() {
        let mut req = make_test_request(&format!("batch-{}", i), prompt, 20);
        req.temperature = 0.7;
