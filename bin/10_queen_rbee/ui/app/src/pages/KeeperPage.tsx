  ...
  return (
    <SidebarProvider>
      <div className="flex h-full w-full">
        <CommandsSidebar onCommandClick={handleCommandClick} activeCommand={activeCommand} />

        {/* Command Output Area */}
        <div className="flex-1 p-4">
          <TerminalWindow title="rbee-keeper" variant="output" copyable copyText={output} className="h-full">
            <div className="whitespace-pre-wrap">
              <span className={activeCommand ? 'text-foreground' : 'text-muted-foreground'}>{output}</span>
            </div>
          </TerminalWindow>
        </div>
      </div>
