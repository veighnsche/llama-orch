...
### ✓ Cross-Reference
- [ ] Compare scores[0, 0, 0, :5] with reference
...
### Step 3: Write Test
```rust
// tests/checkpoint_04_attention_scores.rs
#[test]
fn checkpoint_04_matches_reference() {
    // Load reference
    let reference = load_reference("checkpoint_04_scores.npy");
    
    // Run our implementation
    let scores_layer = AttentionScores::new(64);
    let scores = scores_layer.forward(&q, &k, Some(&mask));
    
    // Compare
    assert_tensors_close(&scores, &reference, 1e-4);
}
```

### Step 4: Validate
```bash
cargo test checkpoint_04
```
