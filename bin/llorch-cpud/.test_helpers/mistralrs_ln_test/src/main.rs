...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Generate IDENTICAL input as llorch-cpud and Candle tests
    ...
    // NOTE: Mistral.rs uses Candle's LayerNorm directly (see mistralrs-core/src/layers.rs:11)
    ...
    println!("(Uses Candle's LayerNorm - see mistralrs-core/src/layers.rs)");
    ...
    let output_str = output_vec
        ...
        .collect::<Vec<_>>()
        .join("\n");
    std::fs::write("/tmp/llorch_test_output_mistralrs.txt", output_str)?;

    eprintln!("\n✅ SUCCESS: Output saved to /tmp/llorch_test_output_mistralrs.txt");
    eprintln!("NOTE: Mistral.rs uses Candle LayerNorm, so results match Candle exactly.");
