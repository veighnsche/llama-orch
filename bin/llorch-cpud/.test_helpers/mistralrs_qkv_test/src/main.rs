//! Mistral.rs QKV Projection Reference Implementation
//!
//! Mistral.rs uses Candle's Linear layer for attention projections.
//! This test validates the same QKV projection logic.
...
fn main() -> anyhow::Result<()> {
    ...
    println!("=== MISTRAL.RS QKV PROJECTION TEST ===");
    println!("(Uses Candle's Linear layer - see mistralrs-core/src/layers.rs)");
    ...
    // 3. Create Linear layer (same as Mistral.rs uses)
    ...
    write_output("checkpoint_02_q_mistralrs.txt", &q_vec)?;
    write_output("checkpoint_02_k_mistralrs.txt", &k_vec)?;
    write_output("checkpoint_02_v_mistralrs.txt", &v_vec)?;
    
    println!("\n✅ Mistral.rs QKV projection complete");
    println!("Output files written to:");
    println!("  - checkpoint_02_q_mistralrs.txt");
    println!("  - checkpoint_02_k_mistralrs.txt");
    println!("  - checkpoint_02_v_mistralrs.txt");
