...
fn test_checkpoint_06_multi_reference() {
    println!("\n╔══════════════════════════════════════════════════════════╗");
    println!("║  Checkpoint 6: Multi-Reference Validation                ║");
    ...
    if ref_path.exists() {
        ...
        if max_diff < 1e-4 {
            println!("\n✅ PYTORCH: FFN matches HuggingFace (max diff {:.6e})", max_diff);
        } else {
        ...
        let candle_path = dir.join("checkpoint_06_ffn_candle.npy");
