...
fn test_layer_norm_determinism_synthetic() {
    ...
    let input =
        Array2::from_shape_fn((batch_size, dim), |(i, j)| ((i * dim + j) as f32 * 0.01).sin());
    ...
    for (i, output) in outputs.iter().enumerate() {
        assert_eq!(output.shape(), first_output.shape(), "Run {} shape mismatch", i + 1);
...
fn test_layer_norm_determinism_different_inputs() {
    ...
    assert_ne!(hash1, hash2, "Different inputs should produce different outputs");
...
fn test_layer_norm_determinism_with_scale_bias() {
    ...
    // Create input
    let input = Array2::from_shape_fn((3, dim), |(i, j)| ((i * dim + j) as f32 * 0.01).sin());
    ...
    let first_hash = hashes[0];
    for (i, &hash) in hashes.iter().enumerate() {
        assert_eq!(hash, first_hash, "Run {} hash mismatch with scale/bias", i + 1);
    }
