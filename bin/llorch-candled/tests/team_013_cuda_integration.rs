...
fn test_cuda_extended_story_generation() -> Result<()> {
    ...
    println!("  CUDA: {:.2} tok/s ({:.2}s for {} tokens)", 
             tokens_per_sec, duration.as_secs_f64(), result.tokens.len());
...
fn test_cuda_performance_benchmark() -> Result<()> {
    ...
    println!("   Result: {:.2} tok/s ({:.1}x faster than CPU)", 
             tokens_per_sec, tokens_per_sec / 3.23);
