...
use crate::integration::harness::{ProcessState, SystemState, TestHarness};
...
pub fn get_all_transitions() -> Vec<StateTransition> {
    vec![
        // Queen transitions
        StateTransition {
            initial: SystemState { queen: ProcessState::Stopped, hive: ProcessState::Stopped },
            command: vec!["queen".to_string(), "start".to_string()],
            expected: SystemState { queen: ProcessState::Running, hive: ProcessState::Stopped },
            description: "Queen: Stopped → Running (hive stays stopped)".to_string(),
        },
        StateTransition {
            initial: SystemState { queen: ProcessState::Running, hive: ProcessState::Stopped },
            command: vec!["queen".to_string(), "stop".to_string()],
            expected: SystemState { queen: ProcessState::Stopped, hive: ProcessState::Stopped },
        ...
        StateTransition {
            ...
            expected: SystemState { queen: ProcessState::Running, hive: ProcessState::Running },
        ...
        StateTransition {
            initial: SystemState { queen: ProcessState::Running, hive: ProcessState::Running },
            command: vec!["hive".to_string(), "start".to_string()],
            expected: SystemState { queen: ProcessState::Running, hive: ProcessState::Running },
...
async fn test_all_state_transitions() {
    ...
    for (i, transition) in transitions.iter().enumerate() {
        ...
        assert_eq!(
            actual_initial, transition.initial,
            "Failed to set up initial state for: {}",
        ...
        assert_eq!(
            actual_final, transition.expected,
...
async fn test_transition_matrix_all_combinations() {
    ...
    for (queen_state, hive_state) in states {
        ...
        let target_state = SystemState { queen: queen_state.clone(), hive: hive_state.clone() };
        ...
        assert_eq!(
            actual, target_state,
            "Failed to reach state: queen={:?}, hive={:?}",
            queen_state, hive_state
        );
