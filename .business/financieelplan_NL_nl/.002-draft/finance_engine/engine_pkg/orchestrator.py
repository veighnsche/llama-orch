...
def validate_and_write_report(config: Dict[str, Any], costs: Dict[str, Any], lending: Dict[str, Any], price_sheet: pd.DataFrame) -> bool:
    return _validate_and_write_report(config, costs, lending, price_sheet)
...
def render_plan(context: Dict[str, Any]) -> None:
    _render_plan(context=context)


def run_pipeline() -> int:
    config, costs, lending, extra, price_sheet, gpu_df = load_inputs()
    write_run_summary()
    has_errors = validate_and_write_report(config, costs, lending, price_sheet)
    if has_errors:
        return 1
