...
pub async fn given_adapter_llamacpp(world: &mut World) { world.push_fact("adapter.llamacpp"); }
...
pub async fn given_adapter_vllm(world: &mut World) { world.push_fact("adapter.vllm"); }

#[then(regex = r"^the adapter implements health/properties/completion/cancel/metrics against vLLM$")]
pub async fn then_adapter_implements_vllm_endpoints(_world: &mut World) {}

#[given(regex = r"^a worker adapter for tgi$")]
pub async fn given_adapter_tgi(world: &mut World) { world.push_fact("adapter.tgi"); }
...
pub async fn given_adapter_triton(world: &mut World) { world.push_fact("adapter.triton"); }
