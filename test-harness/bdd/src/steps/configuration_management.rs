...
async fn config_file_with_port(world: &mut World, port: i32) {
    ...
    world.config_content = Some(format!(
        r#"
[server]
host = "0.0.0.0"
port = {}
...
pub async fn config_with_sensitive_fields(world: &mut World, step: &cucumber::gherkin::Step) {
    ...
    if let Some(docstring) = &step.docstring {
        ...
        tracing::info!(
            "✅ TEAM-129: Config file with {} sensitive fields loaded (has_secrets: {})",
            sensitive_count,
            has_secrets
        );
    } else {
...
fn pool_managerd_starts(world: &mut World) {
    ...
    if world.config_valid {
        ...
        if world.narration_adapter.is_some() {
            ...
            let config_path =
                world.config_file_path.as_deref().unwrap_or("/etc/rbee/pool-managerd.toml");
...
fn narration_confirms(world: &mut World, text: String) {
    ...
    let adapter = world.narration_adapter.as_ref().expect("Narration adapter must be installed");
...
fn narration_includes_validation_duration(world: &mut World) {
    ...
    let validation_events: Vec<_> =
        events.iter().filter(|e| e.action.contains("validate")).collect();

    assert!(validation_events.len() > 0, "Expected validation events");

    for event in validation_events {
        assert!(event.duration_ms.is_some(), "Validation events should include duration_ms");
    }
}
...
fn no_secrets_leaked(world: &mut World) {
    ...
    let events = adapter.captured();
    for event in &events {
        assert!(!event.human.contains("Bearer "), "Bearer tokens must be redacted");
        assert!(!event.human.contains("api_key="), "API keys must be redacted");
    }
}
...
fn narration_links_reload_events(world: &mut World) {
    ...
    let events = adapter.captured();
    let reload_events: Vec<_> = events.iter().filter(|e| e.action.contains("reload")).collect();

    if reload_events.len() > 1 {
        let first_correlation_id = &reload_events[0].correlation_id;
        for event in &reload_events {
            assert_eq!(
                &event.correlation_id, first_correlation_id,
...
fn effective_port_is(world: &mut World, expected: i32) {
    ...
    let port = world
        ...
        .unwrap_or_else(|| world.config_values.get("port").unwrap().parse::<i32>().unwrap());
...
fn narration_redacts_env_vars(world: &mut World) {
    ...
    let events = adapter.captured();
    for event in &events {
        assert!(!event.human.contains("secret-"), "Secrets in env vars must be redacted");
    }
}
...
fn validation_fails_with_error(world: &mut World, expected_error: String) {
    ...
    if let Some(error) = &world.config_validation_error {
        assert!(error.contains(&expected_error), "Error should contain '{}'", expected_error);
    }
}
...
fn narration_includes_error_kind(world: &mut World, error_kind: String) {
    ...
    let events = adapter.captured();
    let error_events: Vec<_> = events.iter().filter(|e| e.error_kind.is_some()).collect();

    assert!(error_events.len() > 0, "Expected error events");

    for event in error_events {
        assert_eq!(
            event.error_kind.as_ref().unwrap(),
            &error_kind,
            "error_kind should be '{}'",
            error_kind
        );
    }
}
...
fn startup_fails_with_exit_code(world: &mut World, expected_code: i32) {
    ...
    assert_eq!(world.exit_code, Some(expected_code), "Exit code should be {}", expected_code);
}
...
fn narration_links_load_reload(world: &mut World) {
    ...
    let events = adapter.captured();
    let config_events: Vec<_> = events.iter().filter(|e| e.action.contains("config")).collect();
...
fn narration_does_not_contain(world: &mut World, text: String) {
    ...
    let events = adapter.captured();
    for event in &events {
        assert!(!event.human.contains(&text), "Narration should not contain '{}'", text);
    }
}
...
fn api_keys_redacted(world: &mut World) {
    ...
    for event in &events {
        assert!(!event.human.contains("api_key="), "API keys must be redacted");
        assert!(!event.human.contains("secret-key-"), "Secret keys must be redacted");
    }
}
...
fn narration_cute_has_emoji(world: &mut World) {
    ...
    let events = adapter.captured();
    let cute_events: Vec<_> = events.iter().filter(|e| e.cute.is_some()).collect();
