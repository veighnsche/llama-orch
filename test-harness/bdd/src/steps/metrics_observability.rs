...
fn response_is_prometheus_format(world: &mut World) {
    ...
    let body = world.last_response_body.as_ref().expect("Response body should be present");
...
fn narration_links_requests_to_metrics(world: &mut World) {
    ...
    let events = adapter.captured();
    let correlation_ids: Vec<_> = events.iter().filter_map(|e| e.correlation_id.as_ref()).collect();
...
fn narration_includes_error_kind(world: &mut World) {
    ...
    let events = adapter.captured();
    let error_events: Vec<_> =
        events.iter().filter(|e| e.action.contains("error") || e.action.contains("fail")).collect();

    for event in error_events {
        assert!(event.error_kind.is_some(), "Error events must include error_kind field");
    }
}
...
fn narration_describes_errors_clearly(world: &mut World) {
    ...
    let events = adapter.captured();
    for event in &events {
        assert!(event.human.len() > 10, "Human field must be descriptive (>10 chars)");
        assert!(event.human.len() <= 100, "Human field must be concise (≤100 chars per ORCH-3305)");
    }
}
...
fn secrets_redacted_in_narration(world: &mut World) {
    ...
    for event in &events {
        ...
        assert!(!event.human.contains("password="), "Passwords must be redacted");
    }
}
...
fn narration_includes_field(world: &mut World, field_name: String) {
    ...
    match field_name.as_str() {
        "device" => {
            assert!(
                events.iter().any(|e| e.device.is_some()),
                "Expected device field in narration"
            );
        }
        "model_ref" => {
            assert!(
                events.iter().any(|e| e.model_ref.is_some()),
                "Expected model_ref field in narration"
            );
        }
        "worker_id" => {
            assert!(
                events.iter().any(|e| e.worker_id.is_some()),
...
fn narration_emitted_by_contains(world: &mut World, text: String) {
    ...
    for event in &events {
        if let Some(emitted_by) = &event.emitted_by {
            assert!(emitted_by.contains(&text), "emitted_by should contain '{}'", text);
        }
    }
}
...
fn narration_emitted_at_recent(world: &mut World, max_age_ms: u64) {
    ...
    for event in &events {
        if let Some(emitted_at_ms) = event.emitted_at_ms {
            ...
            assert!(age_ms <= max_age_ms, "Event emitted_at_ms is too old: {} ms ago", age_ms);
        }
    }
}
...
fn narration_story_request_response(world: &mut World) {
    ...
    let events = adapter.captured();
    let story_events: Vec<_> = events.iter().filter(|e| e.story.is_some()).collect();
    ...
    for event in story_events {
        ...
        assert!(
            story.contains("\"") && story.contains("asked") || story.contains("replied"),
...
fn generate_prometheus_metrics(world: &World) -> String {
    ...
    metrics.push_str(&format!(
        "pool_mgr_workers_total{{status=\"live\"}} {}\n",
        world.workers.values().filter(|w| w.state == "live").count()
    ));
    metrics.push_str(&format!(
        "pool_mgr_workers_total{{status=\"down\"}} {}\n",
