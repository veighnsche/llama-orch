...
pub async fn when_worker_loads_model(world: &mut World) {
    ...
    if !world.model_catalog.is_empty() {
        ...
        tracing::info!("✅ Worker loading model from catalog ({} models available) NICE!", model_count);
...
pub async fn when_worker_reads_gguf_header(world: &mut World) {
    ...
    let gguf_model = world.model_catalog.values()
    ...
    if let Some(model) = gguf_model {
        match std::fs::read(&model.local_path) {
            Ok(bytes) if bytes.len() >= 8 => {
                ...
                if magic == b"GGUF" {
                    tracing::info!("✅ GGUF header read: magic={:?}, version={} NICE!", 
                        std::str::from_utf8(magic).unwrap_or("?"), version);
                    world.last_exit_code = Some(0);
                } else {
...
pub async fn when_calculate_model_size(world: &mut World) {
    ...
    world.node_ram.insert("calculated_model_size".to_string(), (total_size / (1024 * 1024)) as usize);
}
...
pub async fn then_factory_detects_extension(world: &mut World, extension: String) {
    ...
    let has_extension = world.model_catalog.values()
        .any(|entry| {
            entry.local_path.extension()
                .and_then(|s| s.to_str())
...
pub async fn then_factory_creates_quantized_llama(world: &mut World) {
    ...
    let gguf_count = world.model_catalog.values()
        .filter(|entry| {
            entry.local_path.extension()
                .and_then(|s| s.to_str())
                .map(|ext| ext == "gguf")
...
pub async fn then_model_loaded_with_quantized_llama(world: &mut World) {
    ...
    let gguf_models = world.model_catalog.values()
...
pub async fn then_vram_proportional(world: &mut World) {
    ...
    let total_size_mb: u64 = world.model_catalog.values()
        .map(|e| e.size_bytes / (1024 * 1024))
        .sum();
...
pub async fn then_file_size_read(world: &mut World) {
    ...
    assert!(files_read > 0 || !world.model_catalog.is_empty(), 
...
pub async fn then_size_used_for_preflight(world: &mut World) {
    ...
    if let Some(&size_mb) = calculated_size {
        ...
        let total_mb: u64 = world.model_catalog.values()
            .map(|e| e.size_bytes / (1024 * 1024))
            .sum();
...
pub async fn then_size_stored_in_catalog(world: &mut World) {
    ...
    let entries_with_size = world.model_catalog.values()
    ...
    tracing::info!("✅ Size stored in catalog for {}/{} models NICE!", 
        entries_with_size, total_entries);
}
