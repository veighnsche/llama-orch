...
pub async fn then_adapter_implements_llamacpp_endpoints(_world: &mut World) {
    ...
    let h = a.health().expect("health ok");
    assert!(h.live);
    let p = a.props().expect("props ok");
    ...
    let mut stream = a.submit(req).expect("submit ok");
    let _first = stream.next().await;
    a.cancel("t").expect("cancel ok");
    let _v = a.engine_version().expect("engine_version ok");
}
...
pub async fn then_adapter_implements_vllm_endpoints(_world: &mut World) {
    ...
    let _ = a.health().expect("health ok");
    let _ = a.props().expect("props ok");
    let req = api::TaskRequest {
        task_id: "t".into(),
        session_id: "s".into(),
        workload: api::Workload::Completion,
    ...
    let mut s = a.submit(req).expect("submit ok");
    let _ = s.next().await;
    a.cancel("t").expect("cancel ok");
    let _ = a.engine_version().expect("engine_version ok");
}
