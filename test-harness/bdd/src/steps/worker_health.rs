...
use crate::steps::world::{create_http_client, ErrorResponse, World};
    ...
    for worker in idle_workers {
        if let Ok(elapsed) = now.duration_since(worker.last_activity) {
            if elapsed >= timeout_duration {
                ...
                tracing::info!(
                    "✅ Worker {} marked as stale (idle for {:?}) NICE!",
                    worker.id,
                    elapsed
                );
            }
...
pub async fn then_worker_marked_stale(world: &mut World) {
    ...
    let stale_workers: Vec<_> = world.workers.values().filter(|w| w.state == "stale").collect();
...
pub async fn then_worker_removed_from_registry(world: &mut World) {
    ...
    let stale_worker_ids: Vec<String> =
        world.workers.values().filter(|w| w.state == "stale").map(|w| w.id.clone()).collect();
    ...
    for worker_id in &stale_worker_ids {
        assert!(
            !remaining_workers.iter().any(|w| &w.id == worker_id),
...
pub async fn then_emit_warning_log(world: &mut World) {
    ...
    if stale_count > 0 {
        tracing::warn!(
            "⚠️  {} stale worker(s) detected - would emit warning in production",
            stale_count
        );
