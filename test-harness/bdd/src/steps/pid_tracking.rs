...
pub async fn then_hive_logs_force_kill_event(world: &mut World) {
    ...
    assert!(
        has_force_kill_event || world.force_killed_pid.is_some(),
...
pub async fn then_force_kill_log_includes_worker_id(world: &mut World) {
    ...
    let has_worker_id = world
        .audit_log_entries
        .iter()
        .any(|entry| entry.get("details").and_then(|d| d.get("worker_id")).is_some())
...
pub async fn then_force_kill_log_includes_pid(world: &mut World) {
    ...
    let has_pid = world
        .audit_log_entries
        .iter()
        .any(|entry| entry.get("details").and_then(|d| d.get("pid")).is_some())
...
pub async fn then_force_kill_log_includes_reason(world: &mut World) {
    ...
    let has_reason = world
        .audit_log_entries
        .iter()
        .any(|entry| entry.get("details").and_then(|d| d.get("reason")).is_some())
        || world.last_error_message.is_some();

    assert!(has_reason, "Force-kill log missing reason");

    let reason = world.last_error_message.as_ref().map(|s| s.as_str()).unwrap_or("timeout");
...
pub async fn then_force_kill_log_includes_timestamp(world: &mut World) {
    ...
    let has_timestamp =
        world.audit_log_entries.iter().any(|entry| entry.get("timestamp").is_some());

    assert!(
        has_timestamp || world.audit_log_entries.is_empty(),
