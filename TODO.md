...
- [ ] Pool manager minimal readiness backing `/v1/pools/{id}/health`
  - Code: `pool-managerd/src/*`, `orchestratord/src/http/handlers.rs`
- [ ] Structured logs via `tracing` with required fields; redact secrets
  - Code: logging init + fields in handlers; ensure no API keys in logs
- [ ] Worker adapters: mock + llamacpp conformance basics; typed error mapping
  - Code: `worker-adapters/mock/`, `worker-adapters/llamacpp-http/`

### Week 4 — Capabilities verification + scheduling prep

- [ ] Capability discovery snapshots and provider verify
  - Code: new tests under `tools/openapi-client/tests/` or orchestrator provider verify
- [ ] Scheduling placeholders: fairness gauges emission sites; unignore fairness property when ready
  - Code: `orchestrator-core/tests/props_queue.rs`
- [ ] Prep E2E Haiku environment; ensure gating `REQUIRE_REAL_LLAMA=1`
